...
该文档中 \S 4.1 编译方式一节写道：“\CTeX 宏集会根据用户使用的编译方式，在底层选择不同的中文支持方式“（见表 \ref{tb:ctex}）。现在我们可以说，目前使用 \LaTeX 排版中文，推荐的方案是调用 xeCJK/ctex 宏包，并相应使用 \hologo{XeLaTeX} 或 \hologo{LuaLaTeX} 引擎编译生成 PDF 文件。当使用 latexmk 工具（可参见附录 \ref{ap:latexmk}）自动编译时，相应添加参数 `-xelatex' 或 `-lualatex'.
...
% \begin{texlst}[caption=demo-ctex.tex]
